...
bam_file_list = run_dir + "bamlists/CACNA1A_hap.txt"
# bam_file_list = run_dir + "bamlists/ATXN7_hap.txt"
true_available = True
	...
	with open('genot_errors.txt', 'w') as results:
		for record in bam_list:
			...
			if true_available == True:
				real_genot = float(bam_file.split("/")[-1].split(".")[0].split("_")[1])
			...
			cmd = subprocess.Popen([repo_dir + "/src/GangSTR", \
									...
									"--spanweight", "1.0", \
									"--flankweight", str(1),\
									"--ploidy", str(1)], stdout=subprocess.PIPE)
			cmd.wait()
			for line in cmd.stdout:
				# print line
				result = line.rstrip()
				estm_genot = [float(x) for x in list(re.findall('(\d+)\, (\d+)',result))[0]][0]
			if true_available == False:				
				print '>>  ', estm_genot
			else:
				print '>>  ', real_genot, '->\t', estm_genot
				samp_error_s = estm_genot - real_genot
